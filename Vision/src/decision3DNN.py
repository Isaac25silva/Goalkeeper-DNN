...
#===============================================================================
        ...
        if frame.all()==None:
            ...
            type_label, results = inferencia( epoch_r1, dataset1, model1, cv2.cvtColor(frame, cv2.COLOR_BGR2RGB), labels1)
#            print type_label, results

            if type_label == 'ball':
                type_label, results = inferencia( epoch_r2, dataset2, model2, cv2.cvtColor(frame, cv2.COLOR_BGR2RGB), labels2)
            else:
                type_label, results = inferencia( epoch_r3, dataset3, model3, cv2.cvtColor(frame, cv2.COLOR_BGR2RGB), labels3)


            for index in number_label:
                a = list(mem_p[index])
                del a[0]
                mem_p[index] = a
                b = list(mem_p[index])
...
#    net_address = "DNN_ballClass"
    net_address1 = "2class"
    net_address2 = "classBall"
    net_address3 = "noBall"
    path_jobs_dir = '/home/fei/RoboFEI-DNN/Vision/src/nets/3DNN'
    ...
    #read labels
    labels1 = read_labels(path_jobs_dir+'/'+net_address1+'/labels.txt')
    labels2 = read_labels(path_jobs_dir+'/'+net_address2+'/labels.txt')
    labels3 = read_labels(path_jobs_dir+'/'+net_address3+'/labels.txt')
#        print labels
#    number_label1 =  dict(zip(labels1, range(len(labels1))))
#        print number_label

    #buffer_t = np.zeros((10) , dtype=np.int)
    number_label = {'ballCenter': 0, 'noBallIn': 5, 'noBallOut': 6, 'ballRight': 2, 'kickLeft': 3, 'kickRight': 4, 'ballLeft': 1}
#    number_label =  dict(zip(labels, range(len(labels))))
#    print number_label

    #buffer_t = np.zeros((10) , dtype=np.int)
    memory_temp_size = 7
    num_itens = 7 #len(labels)
...
#    os.system("v4l2-ctl -d /dev/video0 -c focus_auto=0 && v4l2-ctl -d /dev/video0 -c focus_absolute=0")
    ...
    try:
        epoch_r1, dataset1, model1 = infer(
            jobs_dir = path_jobs_dir,
            model_id = net_address1,
            epoch = '-1'
        )
    ...
    try:
        epoch_r2, dataset2, model2 = infer(
            jobs_dir = path_jobs_dir,
            model_id = net_address2,
            epoch = '-1'
        )
    except Exception as e:
        logger.error('%s: %s' % (type(e).__name__, e.message))
    ...
    try:
        epoch_r3, dataset3, model3 = infer(
            jobs_dir = path_jobs_dir,
            model_id = net_address3,
            epoch = '-1'
        )
    except Exception as e:
        logger.error('%s: %s' % (type(e).__name__, e.message))
